 ...
 * 查看账户拥有的角色
 */
export function findUserHasRoleApi(
  userId: number
 ...
 * 将某账户设置成为超管
 */
export function userToRootApi(
  userId: number
): Promise<AxiosResponseDataType<string>> {
  return request({
    url: `/user/root/adduseradmin/${userId}`,
    method: "PUT",
    isAuth: true,
  });
}
 ...
 * 将某账号撤销超管身份
 */
export function userRemoveRootApi(
  userId: number
): Promise<AxiosResponseDataType<string>> {
  return request({
    url: `/user/root/revokeuseradmin/${userId}`,
    method: "PUT",
    isAuth: true,
  });
}
